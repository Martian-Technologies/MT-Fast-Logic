...
function BaseFastLogicBlock.server_saveDataToStorage(self)
    self.storage:save(self.data)
end

function BaseFastLogicBlock.server_loadData(self)
...
function BaseFastLogicBlock.server_onProjectile(self, position, airTime, velocity, projectileName, shooter, damage, customData, normal, uuid)
    ...
    -- local layers, LayerHash, outputBlocks, outputHash, farthestOutput = sm.MTFastLogic.BalancedLogicFinder.findBalancedLogic(self.FastLogicRunner, runnrerId)
    -- if layers == nil then
    --     print("No Balanced Logic Found")
    --     return
    -- end
